...
async function loadRankings() {
  ...
  try {
    ...
    if (response.status === 404) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #999;">No students found. Create accounts to see rankings.</td></tr>';
      if (container) container.classList.remove('updating');
      return;
    }
    ...
    showViewMessage(
      `Showing ${result.count} student(s) sorted by ${sortBy.toUpperCase()} rating`,
      'success',
    );
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #dc3545;">Failed to load rankings. Please try again.</td></tr>';
    showViewMessage(
      'Failed to load rankings. Please check your connection and try again.',
      'error',
    );
